...
int openCell(cell grid[][GRIDSIZE], int x, int y)
{
	...
	if(isLocationLegal(x,y) == 1)	/* Opens cell,if it is legal and not flagged */
	{
		...
		{
			for(i=0;i<2;i++)		/*Selected cell's 8 neighbors are checked for emptiness and empty neighbors are  also opened.*/
			{	
				for(j=0;j<2;j++)
				{
					if(grid[x+i][y+j] == closedEmpty && isLocationLegal(x+i,y+j) == 1 &&grid[x+i][y+j] != flaggedEmpty)					
						grid[x+i][y+j] = empty;
					if(grid[x-i][y+j] == closedEmpty && isLocationLegal(x-i,y+j) == 1 &&grid[x-i][y+j] != flaggedEmpty)	
						grid[x-i][y+j] = empty;							
					if(grid[x-i][y-j] == closedEmpty && isLocationLegal(x-i,y-j) == 1 &&grid[x-i][y-j] != flaggedEmpty)					
		...
		result = -2; 		/* return -2,if selected cell is illegal or flagged */
...
int asMain()
{
	...
	int x = GRIDSIZE,y = GRIDSIZE,countMove = 0,countMine = 0,openedCells = 0,continueGame= 1,bomb = 0;
